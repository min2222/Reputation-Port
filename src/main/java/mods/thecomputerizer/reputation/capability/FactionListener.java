...
public class FactionListener extends SimplePreparableReloadListener<Void> {
    ...
    protected void apply(@Nonnull Void value, @Nonnull ResourceManager manager, @Nonnull ProfilerFiller profiler) {
        ...
        try {
            ...
            for(Map.Entry<ResourceLocation, List<Resource>> entry : manager.listResourceStacks("factions",res1 -> res1.getPath().endsWith("json")).entrySet()) {
            ...
            for(Map.Entry<ResourceLocation, List<Resource>> entry : manager.listResourceStacks("chat",res1 -> res1.getPath().endsWith("json")).entrySet()) {
            	ResourceLocation res = entry.getKey();
                if(!checked.contains(res)) {
                    InputStreamReader reader = new InputStreamReader(getResourceStream(manager,res),StandardCharsets.UTF_8);
            ...
            try {
                ...
                if(!manager.getResource(aiRes).isEmpty()) {
